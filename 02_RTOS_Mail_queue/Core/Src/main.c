  ...
  while(1) 
	{
		...
		receive_task1_data = osMailGet(myQueue01Handle, osWaitForever);
		if(receive_task1_data.status == osEventMail) {
			receiver = (DATA_Typedef*)receive_task1_data.value.p;
			mPrint("cnt is: %d\n", receiver->cnt);
